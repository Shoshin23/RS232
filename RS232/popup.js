...
function refreshtable() {
    ...
    if (!bp.updateinprogress && bp.H1.length>bp.LogLines) {
        for (var i=bp.H1.length-bp.LogLines; i<bp.H1.length; i++) {
            ...
            var ti=new Date(bp.tim[i]*bp.TradeFreq*1000) //changed
            var mts=new Date(bp.tim[i]*bp.TradeFreq*1000).getMinutes() //added
            if (mts < 10)  mts='0'+mts;  //added
            r.style.backgroundColor=bcols[((bp.tim[i]+1)/24)&1]
            r.title=wds[ti.getDay()]
            r.insertCell(-1).innerHTML=(new Date(bp.tim[i]*bp.TradeFreq*1000)).getHours() + ":" + mts  //changed
    ...
    if (isNaN(bp.USD) || isNaN(bp.BTC)) {
        ...
        document.getElementById("usd").innerHTML=bp.USD.toFixed(2)
